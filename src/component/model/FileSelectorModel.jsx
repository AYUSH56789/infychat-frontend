...
import { sendAttachmentsApi } from '../../features/message/MessageSlice';
...
export default function FileSelectorModel() {
    ...
    const handleOnFileSubmit = async (event) => {
        event.preventDefault();
    
        try {
            if (selectedFile) {
                const formData = new FormData();
                formData.append('file', selectedFile); // Append selected file with field name 'file'
                console.log(Object.fromEntries(formData),"formdata");
                const resp = await dispatch(sendAttachmentsApi(formData));
                if (resp) {
                    await dispatch(setIsFileSelectorModelOpen(false));
                    toast.success("Your message send successfully.");
                }
            } else if (selectedFiles.length > 0) {
                const formData = new FormData();
                selectedFiles.forEach(file => {
                    formData.append('files', file); // Append each selected file with field name 'files'
                });
                console.log(Object.fromEntries(formData),"formdata",selectedFiles);
                const resp = await dispatch(sendAttachmentsApi(formData));
                if (resp) {  
                    await dispatch(setIsFileSelectorModelOpen(false));  
                }
            } else {
                console.log("Something went wrong.", selectedFile);
            }
        } catch (error) {
            ...
            <div className="mycentered">
                <div className="mymodal">
                    ...
                    <div className="mymodalContent">
                        ...
                        <div>
                            <input type="file" accept="image/*" ref={refDoc} onChange={handleDocFileChange} name="file" style={{ display: 'none' }} />
                        </div>
                        <div>
                            <input type="file" multiple onChange={handleAllFileChange} ref={refAllFiles} name="files" style={{ display: 'none' }} />
                        </div>
