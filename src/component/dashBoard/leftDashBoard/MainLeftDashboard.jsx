...
export default function MainLeftDashboard() {
  ...
  const handleSendMessage = (event, data) => {
    ...
    if (data.type === 'text') {
      ...
      console.log("File is under development.")

    }else{
      console.log("something went wrong....new");
    }
  }
    ...
    <div id="MainLeftDashboard" className={mode.dark ? "darkMode-MainLeftDashBoardbackground" : ""} >
      {showChatContainer === false ? (
        ...
        <>
              ...
              {tempMessage.length === 0 && chatData ? (
                ...
                tempMessage.map((msg, index) => (
                  msg.message.sender._id === data.userId ?
                    <SendChatBox key={index} msgContent={msg.message.messageContent} createdAt={msg.message.createdAt} /> :
                    <ReceiveChatBox key={index} msgContent={msg.message.messageContent} photo={msg.message.sender.photo} createdAt={msg.message.createdAt} />
                ))
              )}

              {allChatMessage.chatMessages.length === 0 ? (
                <div className='noMessageContainer'>
                  <img src="/images/noMessage.svg" height={"45%"} width={"45%"} alt="" />
                </div>
              ) : (
                allChatMessage.chatMessages.map((msg, index) => (
                  msg.sender._id == data.userId ?
                    <SendChatBox key={index} msgContent={msg.messageContent} createdAt={msg.createdAt} /> :
                    <ReceiveChatBox key={index} msgContent={msg.messageContent} photo={msg.sender.photo.url} createdAt={msg.createdAt} />
          ...
          {/* error */}
          <InputText message={message} setMessage={setMessage} handleSendMessage={handleSendMessage} handleOnChangeText={handleOnChangeText} />
        </>
      )}
    </div>
  )
